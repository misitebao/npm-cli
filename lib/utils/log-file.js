...
class LogFiles {
  ...
  constructor ({
    ...
    this.#logId = new Date().toISOString().replace(/[.:]/g, '_')
  ...
  static format (count, level, title, ...args) {
    const prefixes = [count, level]
    if (title) {
      prefixes.push(title)
    }
    const prefix = prefixes.join(' ').trim()
  ...
  async [_cleanLogs] () {
    ...
    try {
      const files = await glob(this[_getLogFileName]('*', '*'))
      if (files.length <= this.#logsMax) {
        return
      }
      for (const file of files.slice(0, this.#logsMax)) {
        try {
          await rimraf(file)
        } catch (e) {
          log.warn('logfile', 'failed to remove log file', file, e)
        }
      }
    } catch (e) {
