  ...
  async [_cleanLogs] () {
    ...
    try {
      ...
      for (const file of files.slice(0, toDelete)) {
        console.error('logfile', 'delete', file)
        try {
          await rimraf(file)
        } catch (e) {
          console.error('logfile', 'error delete', file, e)
          log.warn('logfile', 'error removing log file', file, e)
        }
        console.error('logfile', 'done delete', file)
      }
    } catch (e) {
