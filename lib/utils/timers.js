...
class Timers extends EE {
  ...
  constructor ({ listener, initTimer = 'npm' } = {}) {
    super()
    this.on()
    this.on(listener)
    process.emit('time', initTimer)
    this.started = this.#unfinished.get(initTimer)
  }
        ...
        (f) =>
          ...
          fs.appendFileSync(f, JSON.stringify({
            ...
            unfinished: [...this.#unfinished.entries()].reduce((acc, [name, start]) => {
              acc[name] = { start: start - globalStart, end: globalEnd - globalStart }
              return acc
            }, {}),
          }) + '\n')
      )
