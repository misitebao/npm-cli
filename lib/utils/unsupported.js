...
exports.checkForUnsupportedNode = () => {
  ...
  if (nodejs.unsupported) {
    console.error('npm', 'npm does not support Node.js ' + process.version)
    console.error('npm', 'You should probably upgrade to a newer version of node as we')
    console.error('npm', "can't make any promises that npm will work with this version.")
    console.error('npm', 'You can find the latest version at https://nodejs.org/')
  }
}
