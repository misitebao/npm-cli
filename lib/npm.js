...
class Npm extends EventEmitter {
  ...
  async [_load] () {
    ...
    const notOk = [
      join(this.config.home, '.npm'),
      dirname(__dirname),
    ]

    if (notOk.includes(this.cache)) {
      throw new Error(`bad test cache dir: ${this.cache}`)
    }

    if (notOk.includes(this.prefix)) {
      // throw new Error(`bad test prefix dir: ${this.prefix}`)
    }
