...
class Npm extends EventEmitter {
  ...
  // XXX: should these be made public? For now
  // getters are made available for properties used
  // elsewhere but we should see if that gets difficult
  #timers = null
  #logFile = null
  #display = null
  ...
  constructor () {
    ...
    this.#timers.on((name, ms) => {
      const args = ['timing', name, `Completed in ${ms}ms`]
      this.#logFile.log(...args)
      this.#display.log(...args)
    })
    process.emit('time', 'npm')
    // XXX: This started property is only used in reify output so there
    // is probably a better way to handle that use case
    this.started = this.#timers.unfinished.get('npm')
  ...
  async [_load] () {
    ...
    this.#display.config({
      ...
      progress: this.config.get('progress'),
