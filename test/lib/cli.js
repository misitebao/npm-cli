...
const cliMock = (t, mocks) => {
  withEnvDir(t, 'npm_config_cache')
  const { Npm, outputs, logMocks, logs } = mockNpm(t, mocks)
...
t.test('print the version, and treat npm_g as npm -g', async t => {
  ...
  const { logs, cli, Npm, outputs } = cliMock(t)
  await cli(proc)
